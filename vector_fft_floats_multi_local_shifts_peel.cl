...
__kernel void kernel_func(__local struct cplx* output, __global const float *input, __global const struct cplx *twiddle, __global struct cplx *global_output) {
  ...
  // first butterfly + copy from global to local memory
  {
    struct cplx const twiddle0 = twiddle[0];
    for(int j=0; j<N_LOCAL_BUTTERFLIES; ++j)
    {
      int const idx = 2*(base_idx + j);
      
      // we optimize the butterfly based on the fact that we have real numbers
      
      struct cplx const t = cplxScalarMult(input[idx + 1], twiddle0);
      
      output[idx]   = cplxScalarAdd(input[idx], t);
      output[idx+1] = cplxScalarSub(input[idx], t);
    }
  }
  ...
  for(int i=2, LOG2_N_GLOBAL_BUTTERFLIES_over_i = LOG2_N_GLOBAL_BUTTERFLIES-1;
